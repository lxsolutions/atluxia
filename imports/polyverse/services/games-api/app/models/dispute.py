...
class Dispute(Base):
    ...
    truth_claim_id = Column(String)  # Link to Truth Archive claim
    playful_signal_emitted = Column(Boolean, default=False)
    playful_signal_strength = Column(Numeric(3, 2), default=0.02)  # Max 2% influence
...
class Leaderboard(Base):
    ...
    glicko_rating = Column(Integer, default=1000)
    glicko_rating_deviation = Column(Integer, default=350)  # RD (Rating Deviation)
    glicko_volatility = Column(Numeric(5, 4), default=0.06)  # Volatility parameter
...
class ProofFile(Base):
    ...
    # File information
    filename = Column(String, nullable=False)  # S3 key
    file_url = Column(String, nullable=False)  # s3://bucket/key
    ...
    virus_scan_reason = Column(String)
