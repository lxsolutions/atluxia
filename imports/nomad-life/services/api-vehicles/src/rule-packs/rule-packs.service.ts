...
// Mock implementation for MVP - replace with actual @keyswitch/rules when available
...
class AirportRulePackValidator {
  validateRulePack(rulePackData: any): ValidationResult {
    // Simple validation for MVP
    if (!rulePackData || typeof rulePackData !== 'object') {
      return { valid: false, errors: ['Invalid rule pack data'] };
    }
    
    // Basic validation - return the data for compatibility
    return { 
      valid: true,
      data: rulePackData // Return the original data for compatibility
    };
  }
}
