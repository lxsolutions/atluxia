...
@router.get("/win-probability/{player1_id}/{player2_id}")
def read_win_probability(
    ...
    """
    # Get player ratings
    player1_entry = leaderboard_crud.get_user_rank(db, user_id=player1_id, game_id=game_id)
    player2_entry = leaderboard_crud.get_user_rank(db, user_id=player2_id, game_id=game_id)
    
    if not player1_entry or not player2_entry:
        raise HTTPException(status_code=404, detail="Player ratings not found")
    
    # Calculate win probability
    win_probability = rating_engine.calculate_win_probability(
        player1_entry.elo_rating,
        player2_entry.elo_rating
    )
    ...
    return {
        ...
        "game_id": game_id,
        "player1_rating": player1_entry.elo_rating,
        "player2_rating": player2_entry.elo_rating,
        "player1_win_probability": round(win_probability * 100, 2),
        "player2_win_probability": round((1 - win_probability) * 100, 2)
    }


@router.get("/rating-tiers/{game_id}")
def read_rating_tiers(
    game_id: int,
    db: Session = Depends(get_db)
):
    """
    Get distribution of players across rating tiers
    """
    leaderboards = leaderboard_crud.get_leaderboards(db, game_id=game_id, limit=1000)
    ...
    for entry in leaderboards:
        tier = rating_engine.get_rating_tier(entry.elo_rating)
        tier_distribution[tier] += 1
    
    return {
        "game_id": game_id,
        "total_players": len(leaderboards),
